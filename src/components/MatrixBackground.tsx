...
const MatrixBackground = () => {
  ...
  useEffect(() => {
    ...
    const MOUSE_RADIUS = 150; // raggio d'influenza del mouse
    ...
    const draw = () => {
      ...
      for (let i = 0; i < particles.length; i++) {
        ...
        // Distanza della particella dal mouse
        const distToMouse = Math.hypot(p.x - mx, p.y - my);
        const isNearMouse = distToMouse < MOUSE_RADIUS;

        // Disegna il puntino (più acceso se vicino al mouse)
        ctx.beginPath();
        ctx.arc(p.x, p.y, p.radius, 0, Math.PI * 2);
        ctx.fillStyle = isNearMouse
          ? `hsla(${highlightHue}, ${0.9})`
          : particleColor;
        ctx.fill();
        ...
        for (let j = i + 1; j < particles.length; j++) {
          ...
          if (distance < 120) {
            ...
            if (lineNearMouse) {
              // Intensità in base a quanto è vicina al mouse (più vicina = più accesa)
              const closestDist = Math.min(distToMouse, dist2ToMouse);
              const proximityFactor = 1 - closestDist / MOUSE_RADIUS;
              const alpha = (1 - distance / 120) * 0.5 + proximityFactor * 0.5;
              ctx.strokeStyle = `hsla(${highlightHue}, ${Math.min(alpha, 0.9)})`;
              ctx.lineWidth = 1.2;
            } else {
              const alpha = (1 - distance / 120) * (isDarkMode ? 0.25 : 0.2);
